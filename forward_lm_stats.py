...
@torch.no_grad()
def run_evaluation(device: str, ckpt_file: str, cfg: CustomLLMPagConfig):
    ...
    for batch in tqdm(data_module.val_dataloader(), desc='Forward LM evaluation'):
        ...
        top_k_accuracies = compute_top_k_accuracies(
            inv_first_label=shift_labels[attention_mask == 1].contiguous().view(-1),
            logits=logits[attention_mask == 1].contiguous().view(-1, logits.size(-1)),
            k_samples=4,
            tag='test_forward_lm',
        )
        ...
        for top_k_key, accuracy in top_k_accuracies.items():
            overall_accuracy[top_k_key] += round((accuracy * batch.input_ids.size(0)).item())
