...
def _initialize_hdf5_datasets(
    ...
    if 'created_timestamp' not in f.attrs:
        f.attrs['created_timestamp'] = datetime.datetime.now(datetime.UTC).isoformat()
...
def _update_token_indices(
    ...
    Args:
        ...
        compression_level: Level of compression (0-9, where 9 is highest)
    """
    # Our next_tokens_np is of shape [batch_size * prefix_len, hidden_dim]
    for i in range(next_tokens_np.shape[0]):
        # Cast to Python int for use as key
        token_id = int(next_tokens_np[i])
...
def save_hidden_states_to_hdf5(
    ...
    try:
        with h5py.File(file_path, mode) as f:
            ...
            # Consider each token in the prefix as its own sample
            hidden_states_np = hidden_states_np.reshape(-1, hidden_dim)  # [batch_size * prefix_len, hidden_dim]
            next_tokens_np = next_tokens_np.flatten()  # [batch_size * prefix_len]
            ...
            new_size = current_size + hidden_states_np.shape[0]
            ...
            _update_token_indices(
                f, next_tokens_np, current_size, compression_level
            )

            # Update metadata
            f.attrs['total_samples'] = new_size
            f.attrs['last_updated'] = datetime.datetime.now(datetime.UTC).isoformat()
